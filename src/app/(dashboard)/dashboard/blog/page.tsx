...
type BlogFormSnapshot = Pick<BlogFormState, "title" | "featured_image" | "content" | "category">;
...
function normalizeBlogPost(record: unknown): BlogPost {
    ...
    return {
        ...
        category: String(blog.category ?? "others"),
...
function initialFormState(): BlogFormState {
    return { title: "", featured_image: "", featured_image_file: null, content: "", category: "others" };
}
...
export default function Blog() {
    ...
    function openEditModal(post: BlogPost) {
        ...
        setEditingSnapshot({ title: post.title, featured_image: post.featured_image, content: post.content, category: post.category ?? "others" });
        setFormState({ title: post.title, featured_image: post.featured_image, featured_image_file: null, content: post.content, category: post.category ?? "others" });
    }
    ...
    function buildUpdatePayload() {
        ...
        if (formState.category !== editingSnapshot.category) {
                                ...
                                <div className="grid gap-5">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select
                                            ...
                                            className="h-12 w-full rounded-xl border border-[#D0D5DD] bg-white px-4 text-sm text-[#101828] outline-none transition-colors focus:border-[#EA4335]"
                                        >
